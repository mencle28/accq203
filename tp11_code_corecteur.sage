...
B = C.generator_matrix().transpose().columns() # a completer
...
print("""\
# ****************************************************************************
# DECODAGE DE BERLEKAMP-MASSEY 477
# ****************************************************************************
""")
...
# Code pour l'EXERCICE
#on construit la liste des élements de Fq
n=q-1

#construction de G

lst=[alpha^i for i in range (n)]

liste_linesG=[[lst[i]^j for i in range (n) ] for j in range (k)]
G = matrix(liste_linesG)


#Construction de H
liste_linesH=[[lst[i]^(j+1) for i in range (n) ] for j in range (n-k)]
H = matrix(liste_linesH)


#calcul du syndrome
s = H*r

#calcul du polynome localisateur d'erreur
sigma = PolFq(berlekamp_massey(list(s)[:-1]).reverse())

#on calcul les racines de sigma
M = [x[0] for x in sigma.roots() ] 

#on modifie le message
G_s = G.delete_columns(M)
r_s = vector([r[i] for i in range(len(r)) if i not in M])
...
try:
    m = G_s.solve_left(r_s)
    print( "Le message envoye le plus probable est")
    print( m)
except:
    print("Errreur pas de solution")


reset()
print("""\
# ****************************************************************************
# DECODAGE EN LISTE 482 DONE
# ****************************************************************************
""")
...
print("""\
# ****************************************************************************
# BORNES BATTANT GILBERT-VARSHAMOV 510
# ****************************************************************************
""")
